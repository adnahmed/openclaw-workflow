...
test("PluginOperationRegistry: register and get operation", () => {
	...
	const op = {
		...
		async run(_ctx) {
			return { status: "ok", output_check: { passed: true, decision: "pass", missing_files: [], checked_files: [], validations: [] } };
		},
	};
...
test("PluginOperationRegistry: list returns all registered operations", () => {
	...
	const ids = registry.list().map((o) => o.id).sort();
	assert.deepEqual(ids, ["a", "b"]);
});

test("createDefaultRegistry includes workflow.cache_json_document and workflow.redis_run_initializer", () => {
	const registry = createDefaultRegistry();
	assert.ok(registry.has("workflow.cache_json_document"), "missing cache_json_document");
	assert.ok(registry.has("workflow.redis_run_initializer"), "missing redis_run_initializer");
});
...
test("workflow.cache_json_document: writes artifact from source JSON file", async () => {
	await withTempDir("plugin-cache-json", async (dir) => {
		...
		assert.strictEqual(result.status, "ok", `Expected ok, got: ${result.error}`);

		// Verify artifact was committed
		const artifact = await artifactStore.readArtifact("run-cache-001", "cache-step", "cached-data");
...
test("workflow.cache_json_document: fails gracefully when source_path missing", async () => {
	await withTempDir("plugin-cache-json-missing", async (dir) => {
		...
		const result = await op.run(ctx);
		assert.strictEqual(result.status, "failed", "Should fail when file is missing");
	});
});
...
test("workflow.redis_run_initializer: writes artifact even without redis", async () => {
	await withTempDir("plugin-redis-init", async (dir) => {
		...
		const artifact = await artifactStore.readArtifact("run-init-001", "init-step", "init-artifact");
...
`, "utf8");
		...
		assert.deepEqual(step.with, { source_path: "/tmp/data.json", output_id: "my-data" });
	});
});
...
test("executeWorkflow: plugin step succeeds when operation returns ok", async () => {
	await withTempDir("exec-plugin-ok", async (dir) => {
		...
		const finalState = await executeWorkflow(wf, "run-plugin-ok-001", null, config);
		assert.strictEqual(finalState.status, "ok", `Expected run ok, got: ${finalState.status} — ${finalState.error}`);
...
test("executeWorkflow: plugin step fails when operation not found", async () => {
	await withTempDir("exec-plugin-notfound", async (dir) => {
		...
		const finalState = await executeWorkflow(wf, "run-plugin-missing-001", null, config);
