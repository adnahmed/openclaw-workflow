...
import {
	...
	substituteDeep,
	substituteVars,
} from "../dist/variable-substitution.js";
...
test("substitutes {utc_date} and {utc_datetime}", () => {
	...
	assert.equal(
		substituteVars("UTC date: {utc_date}", ctx),
		"UTC date: 2026-03-09",
	);
	assert.equal(
		substituteVars("UTC datetime: {utc_datetime}", ctx),
		"UTC datetime: 2026-03-09T22:00:00.000Z",
	);
});

test("buildContext includes env with HOME and USERPROFILE fallbacks", () => {
	const ctx = buildContext("run-env");
	assert.ok(ctx.env, "Expected env object in context");
	assert.ok(ctx.env.HOME, "Expected env.HOME to be set");
	assert.ok(ctx.env.USERPROFILE, "Expected env.USERPROFILE to be set");
});
...
test("substitutes {date}", () => {
	...
	assert.equal(
		substituteVars("output-{date}.json", ctx),
		"output-2026-03-09.json",
	);
});
...
test("substitutes {env.X}", () => {
	...
	process.env.OPENCLAW_WORKSPACE = "C:/openclaw/workspace";
	try {
		const ctx = buildContext("run-env-token");
		assert.equal(
			substituteVars("workspace={env.OPENCLAW_WORKSPACE}", ctx),
...
test("keeps escaped env token literal", () => {
	const ctx = buildContext("run-escape");
	assert.equal(substituteVars("\\{env.HOME}", ctx), "{env.HOME}");
});
...
test("substituteDeep processes arrays", () => {
	...
	assert.deepEqual(result, [
		"file-2026-03-09-a.json",
		"file-2026-03-09-b.json",
	]);
});
