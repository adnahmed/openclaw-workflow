...
test('manifest exposes only the workflow tool contract and strict config schema', async () => {
  const manifest = JSON.parse(await readFile(resolve('openclaw.plugin.json'), 'utf8'));
  ...
  assert.equal(manifest.configSchema.properties.concurrency.maximum, 10);
  ...
  for (const key of ['workflowsDir', 'runsDir', 'baseDir', 'concurrency', 'notifyChannel', 'sessionModel', 'pollIntervalMs']) {
