...
import { generateRunId, readRunState, updateRunState, findLatestRun } from './workflow-state.js';
...
export default definePluginEntry({
  ...
  const config = normalizePluginConfig(api?.pluginConfig ?? api?.config?.plugins?.entries?.['openclaw-workflow']?.config ?? {}, api?.runtime ?? {});
  const {
    workflowsDir,
    runsDir,
    baseDir,
  ...
  function buildNotifier() {
    ...
    return async (message) => {
      if (api?.notifications && typeof api.notifications.send === 'function') {
  ...
  async function markBackgroundFailure(runId, err) {
    ...
    try {
      ...
      logger.error(`[workflow:${runId}] failed to persist background failure`, stateErr);
    }
  }
  ...
  api.registerTool({
    ...
    async execute(first, second) {
      const { name, dry_run = false, resume = false } = readParams(first, second);
      try {
        ...
        if (resume) {
          ...
          runInBackground(runId, resumeWorkflow(lastRun, workflow, runId, api, execConfig, runStep));
        ...
        runInBackground(runId, executeWorkflow(workflow, runId, api, execConfig, runStep));
    ...
    async execute(first, second) {
      ...
      try {
        ...
        if (run_id) {
          ...
          if (!state) return errorResult(`No runs found for workflow "${name}"`);
        } else {
        ...
        if (err?.code === 'ENOENT') return errorResult(`Run not found: ${run_id || name}`);
        return errorResult(err instanceof Error ? err.message : String(err));
      }
    },
  });
  ...
  api.registerTool({
    name: 'workflow_list',
    description: 'List available YAML/JSON workflow definitions and their most recent run status.',
  ...
  api.registerTool({
    ...
    description: 'Cancel a running workflow. Running steps may finish, but no new steps will be launched.',
