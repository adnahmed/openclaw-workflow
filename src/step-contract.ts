...
function outputPathList(outputs: OutputSpec[] | string[] | undefined): string[] {
  if (!outputs) return [];
  return outputs.map((o: any) => outputPathOf(o));
}

function cacheManifestFilePath(baseDir: string, stepId: string, outputs: string[]): string {
  const sanitized = stepId.replace(/[^a-zA-Z0-9_-]+/g, "_");
  const fingerprint = createHash("sha256")
    .update(outputs.slice().sort().join("|"))
    .digest("hex")
    .slice(0, 12);
  return join(baseDir, ".openclaw-workflow-cache", `${sanitized}-${fingerprint}.json`);
}
...
async function readSchemaForValidator(
  ...
  if (!validator?.schema) return { schema_hash: hashOf(null), schema_repr: null };
...
async function buildInputSignature(args: {
  ...
  const files: Array<{ path: string; exists: boolean; size?: number; mtimeMs?: number }> = [];
  ...
  for (const depId of step.depends_on || []) {
    ...
    for (const out of declared as any[]) {
      ...
      try {
        ...
        files.push({ path: abs, exists: true, size: st.size, mtimeMs: st.mtimeMs });
      } catch {
...
export async function computeStepContractSignature(args: {
  ...
  const { workflow, step, state, baseDir, workflowsDir, outputsOverride } = args;
  const include =
    step.reuse_outputs?.freshness?.include ||
    [
...
export function evaluateReuseCondition(args: {
  ...
  if (!whenExpr || String(whenExpr).trim().length === 0) return { allowed: true };
...
export function handoffMatchesCurrentAttempt(args: {
  ...
  const { stepState, attempt, session_key, subagent_run_id, handoff_token } = args;
...
export async function adoptStepContract(args: {
  ...
  const { state, runsDir, stepId, outputCheck, reason, message, metadata, counters } =
    args;
